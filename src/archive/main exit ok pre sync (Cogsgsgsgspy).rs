...
fn initialize_uma_application() {
    ...
    // Create or overwrite sync_ok.txt with '0' 
    // let sync_flag_path = Path::new("project_graph_data/session_state_items").join("ok_to_start_sync_flag.txt");
    // OpenOptions::new()
    //     .write(true)
    //     .create(true) 
    //     .truncate(true) // This line will overwrite the file's contents if it already exists
    //     .open(sync_flag_path)
    //     .expect("Failed to open/create sync_ok.txt")
    //     .write_all(b"0") // Write '0' as bytes
    //     .expect("Failed to write to sync_ok.txt");
...
fn sync_flag_ok_or_wait(wait_this_many_seconds: u64) {
    ...
    loop {
        ...
        if file_content.trim() == "0" {
            println!("'continue_uma.txt' is 0. sync_flag_ok_or_wait Exiting loop.");
            break; 
        }
...
fn out_request_sync_loop() {
    loop {
        ...
        // 2. break loop if continue=0
        if file_content.trim() == "0" {
            println!("'continue_uma.txt' is 0. out_request_sync_loop Exiting loop.");
            break; 
        }
    ...
    println!("Finish: out_request_sync_loop");
}
...
fn in_queue_sync_loop() {
    loop {
        ...
        // 2. break loop if continue=0
        if file_content.trim() == "0" {
            println!("'continue_uma.txt' is 0. in_queue_sync_loop Exiting loop.");
            break; 
        }
        ...
        let sync_flag_path = Path::new("project_graph_data/session_state_items/ok_to_start_sync_flag.txt"); 
    ...
    println!("Finish: in_queue_sync_loop");
}
...
fn we_love_projects_loop() -> Result<(), io::Error> {
    ...
    */
    ...
    loop {
        ...
        // 2. break loop if continue=0
        if file_content.trim() == "0" {
            println!("'continue_uma.txt' is 0. we_love_projects_loop() Exiting loop.");
            break; 
        }
    ...
    println!("Finish: we love project loop.");
    Ok(())
}
...
fn set_sync_start_ok_flag_to_true() { 
    if fs::remove_file(SYNC_START_OK_FLAG_PATH).is_ok() {
        // println!("Old 'continue_uma.txt' file deleted."); // Optional log.
    }
...
fn main() {
    ...
    */
    ...
    println!("All threads have completed. UMA says fare well and strive.");
}
