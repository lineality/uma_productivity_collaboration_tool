...
struct SyncCollaborator {
    ...
    sync_interval: u64, 
    signal_port: u16,        // Add the signal port
}
...
fn load_collaborator_list() -> Result<Vec<Collaborator>, MyCustomError> {
    /*
    reads individual user files
    */
    let mut collaborators = Vec::new();
    let dir_path = Path::new("project_graph_data/collaborator_tomls");
...
fn initialize_uma_application() {
    ...
    /*
    TODO: collaborators.toml may be replaced by project_graph_data/collaborator_tomls
    advantages:
    collaborators can gpg sign their own doc
    get need when need: only read what you need when you need it
...
// fn load_teamchannel_connection_data(channel_name: &str) -> Result<HashSet<IpAddr>, io::Error> {
//     /*
...
//             ip_allowlist.insert(IpAddr::V4(ipv4));
//         }
//         if let Some(ipv6) = collaborator.ipv6_address {
//             ip_allowlist.insert(IpAddr::V6(ipv6));
//         }
...
// fn load_teamchannel_connection_data() -> Result<HashSet<SocketAddr>, io::Error> { 
//     /*
//     step 1: get team_channel_members from project_graph_data/session_items/current_node_collaborators_with_access.toml
//     step 2: get current user project_graph_data/session_items/uma_local_owner_user.txt
//     stem 3: remove current user from IP allowlist
...
//     Why Immutability is Crucial Here:
    
//     Single-Purpose Function: The sole purpose of load_teamchannel_connection_data is to 
//     construct and return a allowlist. There is no logical reason to modify 
//     it within the function after it's been created.
    
//     Preventing Accidental Modification: Making it immutable eliminates the 
//     possibility of accidentally altering the allowlist within the function, 
//     safeguarding against incorrect or potentially malicious modifications.
    
//     Whitelist Integrity: An IP allowlist is a sensitive security element. 
//     Its contents should be carefully controlled. By making it immutable within 
//     this function, we reinforce the principle that this data should be treated
//      with care.
...
//     // 4. Extract Socket Addresses, EXCLUDING current_user:
...
//     //         if let Some(ipv6) = collaborator.ipv6_address {
//     //             let socket_addr = SocketAddr::new(IpAddr::V6(ipv6), collaborator.signal_port); 
//     //             sync_config_data_set.insert(socket_addr);
//     //         }  
//     //     } 
//     // }
//     // 4. Extract variousl connection data, EXCLUDING current_user:
//     for member_username in &channel_members {
//         // Find the member in the list of all collaborators:
//         let collaborator = load_collaborator_by_username(member_username)?;
...
//                     ipv6_address,
//                     file_transfer_port: collaborator.file_transfer_port,
...
fn load_collaborator_by_username(username: &str) -> Result<Collaborator, MyCustomError> {
    let toml_file_path = Path::new("project_graph_data/collaborator_tomls")
...
fn load_teamchannel_connection_data() -> Result<HashSet<SyncCollaborator>, MyCustomError> { 
    ...
    project_graph_data/collaborator_tomls/NAME__collaborator.toml

    (note: members should have a list of ipv4, ipv6 addresses, not just one)

    sample: project_graph_data/collaborator_tomls/alice__collaborator.toml
    [[collaborator]]
    user_name = "alice"
    ipv4_addresses = ["24.0.189.112", "24.0.189.112"]
    ipv6_addresses = ["2601:80:4803:9490::2e79","2601:80:4803:9490::2e79"]
    ...
    sample: project_graph_data/collaborator_tomls/bob__collaborator.tomloml
    [[collaborator]]
    user_name = "bob"
    ipv4_addresses = ["24.0.189.112", "24.0.189.112"]
    ipv6_addresses = ["2601:80:4803:9490::2e79","2601:80:4803:9490::2e79"]
        ...
        if collaborator.user_name != current_user {
            if let Some(ipv6_address) = collaborator.ipv6_address {
                sync_config_data_set.insert(SyncCollaborator {
                    ...
                    file_transfer_port: collaborator.file_transfer_port,
                    sync_interval: collaborator.sync_interval,
                    signal_port: collaborator.signal_port,
                });
            } 
        } 
    }
...
//     // Declare socket_addr_allowlist here, before the for-loop: for collaborator in filtered_collaborators {
//     let mut socket_addr_allowlist = HashSet::new();
    
//     // let mut ip_allowlist = HashSet::new(); // should not be mut
//     for collaborator in filtered_collaborators {
//         // if let Some(ipv4) = collaborator.ipv4_address {
//         //     ip_allowlist.insert(IpAddr::V4(ipv4));
//         // }
//         if let Some(ipv6) = collaborator.ipv6_address {
//             let socket_addr = SocketAddr::new(IpAddr::V6(ipv6), collaborator.signal_port); 
//             socket_addr_allowlist.insert(socket_addr);
//         }
//         // ... (Handle multiple IP addresses per collaborator if needed)
//     }
...
// fn is_ip_allowlisted(ip: &IpAddr, allowlist: &Vec<Collaborator>) -> bool {
//     allowlist.iter().any(|c| {
//         match ip {
...
// fn is_ip_allowlisted(ip: &IpAddr, allowlist: &HashSet<IpAddr>) -> bool {
...
//     // Your logic to detect collisions 
//     // ... (Assuming you're comparing ports from collaborators here) 

//     // Example:

//     // 1. Get the relevant collaborator data
//     //    - This could involve loading channel collaborators or current collaborator, etc. 
//     let relevant_collaborators = /* Your logic to get collaborators to check */;

//     // 2. Iterate through the collaborators and their ports
//     for collaborator in relevant_collaborators {
//         // Get this collaborator's signal port:
//         let current_port = collaborator.signal_port;

//         // 3. Check if the port is already in use (by another collaborator)
//         //    - Implement your collision detection logic here 
//         if port_is_used_by_another_collaborator(current_port, &relevant_collaborators) {
//             return Some(collaborator.user_name.clone()); // Return the colliding username
//         }
//     }

//     // If no collision is found after checking all collaborators:
//     None
// } 
...
fn port_is_used_by_another_collaborator(port: u16, collaborators: &HashSet<SyncCollaborator>) -> bool { 
    collaborators.iter().any(|c| c.file_transfer_port == port)
}
...
//         // Load the sync_config_data_set once outside the loop
//         let sync_config_data_set = load_teamchannel_connection_data().unwrap_or_else(|e| { 
//             eprintln!("Error loading sync_config_data_set: {}", e);
//             Vec::new()
//         });    

//         let target_username = get_next_sync_request_username(&sync_config_data_set);
        
//         // Handle the case where target_username is None (no sync request found)
//         if let Some(ref username) = target_username { 
//             // 2. Find the target collaborator in the sync_config_data_set (now using 'username')
//             if let Some(collaborator) = sync_config_data_set.iter().find(|&c| c.user_name == target_username) {
...
//                     Some(username) => println!("Target collaborator not found in sync_config_data_set: {}", username),
...
// #[derive(Debug, Clone, Hash, PartialEq, Eq)]
// struct SyncCollaborator {
//     user_name: String, // Add user_name for easy retrieval later
//     ipv6_address: Ipv6Addr,
//     file_transfer_port: u16,
//     sync_interval: u64, 
// }
    ...
    loop {
        ...
        if let Some(target_collaborator) = maybe_target_collaborator {
            let target_addr = SocketAddr::new(
                ...
                target_collaborator.file_transfer_port
            ); 
...
// fn get_next_sync_request_username(allowlist: &Vec<Collaborator>) -> Option<String> {
//     let mut rng = rand::thread_rng(); // Create a random number generator
//     let mut shuffled_allowlist = allowlist.clone(); // Create a copy to shuffle

//     // Get the username of the first collaborator in the shuffled list
//     shuffled_allowlist.shuffle(&mut rng); // Shuffle the collaborators randomly 

//     // Get the username of the first collaborator in the shuffled list
//     shuffled_allowlist.first().map(|c| c.user_name.clone()) 
// }
