...
impl App {
    ...
    */
    ...
    fn load_im_messages(&mut self) {
        ...
        if self.current_path.is_dir() {
            ...
            (entries.len() == 1 && entries[0].path().file_name().unwrap() == OsStr::new("0.toml")) {
                ...
                debug_log!("This channel is empty. Write a welcoming message:");
    ...
    fn display_error(&mut self, message: &str) {
        ...
        debug_log!("Error: {}", message); // Example: Print the error to the console for now
    }
...
fn add_collaborator_qa(
    ...
    debug_log!("Collaborator '{}' added!", collaborator.user_name); 
...
impl GraphNavigationInstanceState {
    ...
    //     let node_toml_path = self.current_full_file_path.join("node.toml");
        ...
        //             debug_log!("Error loading node.toml: {}", e);
...
//     let collaborator_list = dir_at_path_is_empty_returns_false("project_graph_data/collaborator_tomls");
...
fn initialize_uma_application() {
    ...
    if number_of_team_channels == 0 {
        ...
        debug_log!("There are no existing team channels. Let's create one.");
        debug_log!("Enter a name for the team channel:");
    ...
    */
    ...
    // if !dir_at_path_is_empty_returns_false("project_graph_data/collaborator_tomls") {
    if !dir_at_path_is_empty_returns_false(Path::new("project_graph_data/collaborator_tomls")) { 
        // If there are no existing users, prompt the user to add a new user
        debug_log!("Welcome to the application!");
        debug_log!("To get started, please add a new user.");

        // Prompt the user to enter a username
        debug_log!("Enter a username:");
        let mut username = String::new();
        io::stdin().read_line(&mut username).unwrap();
        let username = username.trim().to_string();

        // // Prompt the user to enter an IP address
        // debug_log!("Enter an ipv4_address:");
      ...
      // Prompt for IPv6 addresses (potentially multiple):
        ...
        loop { 
            debug_log!("Enter an IPv6 address (or 'done' if finished):"); 
            ...
            match ipv6_input.parse::<Ipv6Addr>() { 
                ...
                Err(_) => {
                    debug_log!("Invalid IPv6 address. Please try again.");
                }
            }
        }
      ...
      // Prompt for IPv6 addresses (potentially multiple):
        ...
        loop { 
            debug_log!("Enter an IPv4 address (or 'done' if finished):"); 
            ...
            match ipv4_input.parse::<Ipv4Addr>() { 
                ...
                Err(_) => {
                    debug_log!("Invalid IPv4 address. Please try again.");
                }
            }
        }
                        
        // // Prompt the user to enter an IP address
        // debug_log!("Enter an ipv6_addresses:");
    ...
    if !uma_toml_path.exists() {
        ...
        debug_log!("Welcome to Uma! Please enter your username (this will be the owner for this Uma instance):");
    ...
    if let Some(command) = parts.first() {
        match command.to_lowercase().as_str() {
            ...
            "make node" | "new node" | "add node" => {
                ...
                debug_log!("Enter a name for the new node:");
                ...
                debug_log!("Enter a description for the new node:");
                ...
                debug_log!("Enter collaborators (comma-separated usernames):");
                ...
                debug_log!("Enter the order number for the new node:");
                ...
                debug_log!("Enter the priority for the new node (High, Medium, Low):");
                ...
                let priority = match priority_input.trim().to_lowercase().as_str() {
                    ...
                    _ => {
                        debug_log!("Invalid priority. Defaulting to Medium.");
                        NodePriority::Medium
                    }
                };
...
fn send_hello_signal(target_addr: SocketAddr) -> Result<(), io::Error> {
    match TcpStream::connect(target_addr) {
        Ok(mut stream) => {
            ...
            debug_log!("Sent 'Hello, UMA!' signal to {}", target_addr);
            Ok(())
        }
        Err(e) => {
            debug_log!("Failed to send signal to {}: {}", target_addr, e);
            Err(e) 
        }
    }
}
            ...
            match TcpStream::connect(target_addr) { 
                Ok(mut stream) => {
                    ...
                    if let Err(e) = stream.write_all(toml_data.as_bytes()) {
                        debug_log!("Error sending TOML data to {}: {}", target_addr, e);
                    } else {
                        debug_log!("Sent TOML file to {}", target_addr);
                        // 5. Optionally receive a response signal (e.g., Sync Done)
                        // ... (Implement response signal logic here)
                    }
                }
                Err(e) => {
                    debug_log!("Error connecting to target instance {}: {}", target_addr, e);
                }
            }
...
fn in_queue_sync_loop() {
    loop {
        ...
        let allowlist = load_teamchannel_connection_data().unwrap_or_else(|e| { 
            debug_log!("Error loading allowlist: {}", e);
            HashSet::new()
        });
                ...
                if is_ip_allowlisted(&addr.ip(), &allowlist) {
                    ...
                    match signal.trim() {
                        "Sync Request" => {
                            ...
                            debug_log!("Received Sync Request from {}", addr.ip());
                        }
                        _ => {
                            debug_log!("Received unknown signal: {}", signal);
                        }
                    }
                    ...
                    debug_log!("Connection rejected from non-allowlisted IP: {}", addr.ip());
                }
            }
            Err(e) => {
...
fn you_love_file_sync_base_loop() {
    ...
    */
    ...
    if let Err(err) = in_queue_sync_loop_handle.join() {
        debug_log!("Error joining in_queue_sync_loop: {:?}", err);
    }
    if let Err(err) = out_request_sync_loop_handle.join() {
        debug_log!("Error joining out_request_sync_loop: {:?}", err);
    }
}
...
fn we_love_projects_loop() -> Result<(), io::Error> {
    ...
    */
    ...
    //     debug_log!("GPG key verification failed (placeholder)");
...
fn main() {
    ...
    */
    ...
    debug_log!("All threads have completed. UMA says fare well and strive.");
}
